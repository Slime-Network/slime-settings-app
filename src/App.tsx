			...
			<Grid container>
					...
					<Stack spacing={2} margin={2}>
						<Paper sx={{ p: 2 }}>
							<Grid container spacing={4}>
								...
								<Grid item xs={9}>
									<TextField
										id="Profile Update fee tf"
										sx={{ width: '100%' }}
										variant="filled"
										type="number"
										label="Profile Update Fee (mojo)"
										value={fee}
										onChange={(e) => {
											const regex = /^[0-9\b]+$/;
				...
				<Grid item xs={9}>
					{!session?.acknowledged && (
						<Typography variant="h6" component="h2" sx={{ margin: 5 }}>
							Connect to your wallet to see your profiles
						</Typography>
					)}
					{session && !loaded && <CircularProgress sx={{ margin: 5 }} />}
					{session && loaded && profiles.length <= 0 && (
						<Typography variant="h6" component="h2" sx={{ margin: 5 }}>
							No profiles found
						</Typography>
					)}
										...
										<Grid container>
											<Grid item xs={12}>
												<Typography variant="h6" component="h2">
													...
													<Chip
														size="small"
														label={gostiConfig.identity.activeDID === profile.did ? 'Active' : 'Activate'}
														color={gostiConfig.identity.activeDID === profile.did ? 'primary' : 'default'}
														onClick={() => {
